"""
Part C: Experiment with neural network depth.
Vary the hidden layer architectures: {512}, {512, 256}, {512, 256, 128}, {512, 256, 128, 64}

STOPPING CRITERION:
-------------------
Same as Part B: Loss change < 0.001 or max 250 epochs
"""
...
# Most functions are similar from part b
# In part b, we had varying hidden units in a single hidden layer
# Here, we vary the number of hidden layers (depth) and their units
# Hence, we reuse many functions from part b with minor modifications
# load_images_from_folder, evaluate_metrics, print_metrics_table, and save_predictions_to_csv remain the same
# other functions are modified to handle varying depth
...
def print_metrics_table(metrics, dataset_name, architecture):
    ...
    print(f"\n{'='*80}")
    print(f"{dataset_name} - Architecture: [{architecture}]")
    print(f"{'='*80}")
...
def experiment_network_depth(X_train, y_train, X_test, y_test, architectures):
    ...
    for architecture in architectures:
        ...
        print(f"\n{'#'*80}")
        print(f"# Training with architecture: [{arch_str}] (Depth: {depth})")
        print(f"{'#'*80}")
...
def plot_f1_vs_depth(results, save_path='f1_vs_depth.png'):
    # plot average F1 score vs network architecture depth
    ...
    plt.title('Average F1 Score vs Network Depth', fontsize=14, fontweight='bold')
    ...
    for i in range(len(x_pos)):
        plt.text(x_pos[i], results['train_avg_f1'][i], f"{results['train_avg_f1'][i]:.3f}", 
...
def save_detailed_results(results, filename='detailed_results_depth.txt'):
    # Save detailed results to a text file.
    with open(filename, 'w') as f:
        f.write("="*80 + "\n")
        f.write("DETAILED RESULTS - VARYING NETWORK DEPTH\n")
        f.write("="*80 + "\n\n")
...
def main():
    ...
    print("="*80)
    print("PART C: NEURAL NETWORK WITH VARYING DEPTH")
    print("="*80)
    print("\nSTOPPING CRITERION: Loss change < 0.001 or max 250 epochs")
    print("="*80)
    ...
    # Run experiments
    results, all_test_predictions = experiment_network_depth(X_train, y_train, X_test, y_test, architectures)
    
    # Save predictions to CSV in output folder
    prediction_file = os.path.join(output_folder_path, 'prediction_c.csv')
    save_predictions_to_csv(all_test_predictions, prediction_file)
    
    # Save detailed results in output folder
    results_file = os.path.join(output_folder_path, 'detailed_results_depth.txt')
    save_detailed_results(results, results_file)
    
    # Plot results and save in output folder
    plot_file = os.path.join(output_folder_path, 'f1_vs_depth.png')
    ...
    print("\nOBSERVATIONS:")
    print("-" * 80)
    print("1. Impact of Network Depth on Performance:")
    ...
    print("   - Very deep networks may face vanishing gradient or overfitting issues")
    
    print("\n2. Training Characteristics:")
    print("   - Deeper networks typically require more training time")
    print("   - Convergence behavior may differ across architectures")
    for i, arch in enumerate(results['architectures']):
        arch_str = '-'.join(map(str, arch))
    ...
    print("\n5. Depth vs Performance Trade-off:")
    print("   - Increasing depth may improve performance up to a point")
    print("   - Beyond optimal depth, diminishing returns or degradation may occur")
